...
BEGIN
    CREATE TABLE pieces_of_luggage (
		...
		ticket_code VARCHAR(50), -- Relación muchos a uno con ticket
		FOREIGN KEY (ticket_code) REFERENCES ticket(ticketing_code)
    );
...
-- tabla reservation
IF NOT EXISTS (SELECT * FROM sys.objects WHERE name = 'reservation' AND type = 'U')
BEGIN
    CREATE TABLE reservation (
        id_reservation UNIQUEIDENTIFIER DEFAULT NEWID() PRIMARY KEY,
        reservation_date DATETIME NOT NULL,
        status VARCHAR(50) NOT NULL,
        ticket_code VARCHAR(50) UNIQUE, -- Relación uno a uno con ticket
        flight_id UNIQUEIDENTIFIER NOT NULL, -- Relación muchos a uno con flight
        FOREIGN KEY (ticket_code) REFERENCES ticket(ticketing_code) ON DELETE CASCADE,
        FOREIGN KEY (flight_id) REFERENCES flight(id) ON DELETE CASCADE
    );
...
-- tabla boarding_pass
IF NOT EXISTS (SELECT * FROM sys.objects WHERE name = 'boarding_pass' AND type = 'U')
BEGIN
    CREATE TABLE boarding_pass (
        id_boarding_pass UNIQUEIDENTIFIER DEFAULT NEWID() PRIMARY KEY,
        gate VARCHAR(50) NOT NULL,
        status VARCHAR(50) NOT NULL,
        boarding_time DATETIME NOT NULL,
        seat_id UNIQUEIDENTIFIER UNIQUE, -- Relación uno a uno con seat
        ticket_code VARCHAR(50) UNIQUE, -- Relación uno a uno con ticket
        FOREIGN KEY (seat_id) REFERENCES seat(id) ON DELETE CASCADE,
        FOREIGN KEY (ticket_code) REFERENCES ticket(ticketing_code) ON DELETE CASCADE
    );
...
-- Índices adicionales para optimización de consultas con verificación de existencia

-- Verifica si el índice no existe antes de crearlo
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_City_Country' AND object_id = OBJECT_ID('city'))
BEGIN
    CREATE INDEX IDX_City_Country ON city(country_id);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_Airport_City' AND object_id = OBJECT_ID('airport'))
BEGIN
    CREATE INDEX IDX_Airport_City ON airport(city_id);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_Flight_FlightNumber' AND object_id = OBJECT_ID('flight'))
BEGIN
    CREATE INDEX IDX_Flight_FlightNumber ON flight(flight_number_id);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_AvailableSeat_Seat' AND object_id = OBJECT_ID('available_seat'))
BEGIN
    CREATE INDEX IDX_AvailableSeat_Seat ON available_seat(seat_id);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_Passport_Customer' AND object_id = OBJECT_ID('passport'))
BEGIN
    CREATE INDEX IDX_Passport_Customer ON passport(customer_id);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_IdentityDocument_Customer' AND object_id = OBJECT_ID('identity_document'))
BEGIN
    CREATE INDEX IDX_IdentityDocument_Customer ON identity_document(customer_id);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_Coupon_Ticket' AND object_id = OBJECT_ID('coupon'))
BEGIN
...
BEGIN
    ...
    JOIN sys.indexes AS i ON i.object_id = indexstats.object_id
    WHERE i.type > 0 -- Excluye índices de tipo heap (sin índices)
    AND indexstats.avg_fragmentation_in_percent > 10 -- Fragmentación mínima para actuar
    AND i.is_disabled = 0; -- Excluir índices deshabilitados
    ...
    BEGIN
        IF @SQL < 30 -- Fragmentación menor al 30%, reorganizar
            SET @SQL = N'ALTER INDEX ' + QUOTENAME(@IndexName) + ' ON ' + QUOTENAME(@TableName) + ' REORGANIZE;';
        ELSE -- Fragmentación mayor o igual al 30%, reconstruir
...
-- Poblar base de datos con nuevas tablas y relaciones
BEGIN TRY
    -- Iniciar la transacción
    BEGIN TRANSACTION;

 -- Insertar reservas
	IF NOT EXISTS (SELECT * FROM reservation WHERE reservation_date = '2024-08-15')
    BEGIN
        INSERT INTO reservation (reservation_date, status, ticket_code, flight_id) 
        VALUES 
            ('2024-08-15', 'Confirmada', 
             (SELECT ticketing_code FROM ticket WHERE ticketing_code = 'BOLETO001'), 
             (SELECT id FROM flight WHERE flight_date = '2024-09-01')),
            ('2024-08-16', 'Cancelada', 
             (SELECT ticketing_code FROM ticket WHERE ticketing_code = 'BOLETO002'), 
             (SELECT id FROM flight WHERE flight_date = '2024-09-02'));
    END

    -- Insertar pases de abordar
    IF NOT EXISTS (SELECT * FROM boarding_pass WHERE gate = 'A1')
    BEGIN
        INSERT INTO boarding_pass (gate, status, boarding_time, seat_id, ticket_code) 
        VALUES 
        ('A1', 'Validado', '2024-09-01 07:45:00', (SELECT id FROM seat WHERE location = '1A'), (SELECT ticketing_code FROM ticket WHERE ticketing_code = 'BOLETO001')),
        ('B2', 'No Validado', '2024-09-02 08:45:00', (SELECT id FROM seat WHERE location = '2B'), (SELECT ticketing_code FROM ticket WHERE ticketing_code = 'BOLETO002'));
    END
